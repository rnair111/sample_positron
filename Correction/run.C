...
void run(TString runMode = "full")
{
    ...
    //Analysis Manager
    AliAnalysisManager* mgr = new AliAnalysisManager("correctionAnalysis");
    ...
    AliAnalysisDataContainer *coutputA = mgr->CreateContainer( "AnalysisResultData", TList::Class( ), AliAnalysisManager::kOutputContainer, "AnalysisResultMC.root" );
    ...
    mgr->ConnectOutput( task, 1, coutputA );
     ...
     if (local) {
        ...
        chain->Add("alien:///alice/sim/LHC11a10a_bis/139465/AOD162/1589/AliAOD.root");
...
//      if (!alienHandler) return;
        ...
        alienHandler->SetAliPhysicsVersion("vAN-20220823_ROOT6-1");
        //alienHandler->SetGridDataDir("/alice/data/2010/LHC10h");
        //alienHandler->SetDataPattern("/ESDs/pass2/AOD160/*AOD.root");
        alienHandler->SetGridDataDir("/alice/sim/LHC11a10a_bis/");
        alienHandler->SetDataPattern("/AOD162/*/AliAOD.root");
        ...
        alienHandler->SetGridWorkingDir("PbPb_MC");
    ...
    //    alienHandler->SetOutputFiles("AnalysisResultData.root");
        alienHandler->SetOutputFiles("AnalysisResultMC.root");
...
void SetupEnvironment()
{
    ...
    R__LOAD_LIBRARY( libVMC.so );
    R__LOAD_LIBRARY( libPhysics.so );

    //load analysis framework
//    gSystem->Load( "libANALYSIS" );
//    gSystem->Load( "libANALYSISalice" );
    R__LOAD_LIBRARY(libESD.so);
    R__LOAD_LIBRARY(libAOD.so);
    R__LOAD_LIBRARY(libPhysics.so);
    R__LOAD_LIBRARY(libGeom.so);
    R__LOAD_LIBRARY(libANALYSIS.so);
    R__LOAD_LIBRARY(libANALYSISalice.so);
    //add include path
